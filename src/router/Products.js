export const Products = [
  {
    id: 1,
    name: "Mobile Tripod 3110",
    price: "Rs. 1,750",
    description:
      "3110 Portable Camera Tripod with Three-dimensional Head & Carrying Bag for Canon Nikon Sony Cameras Camcorders (Silver)",
    category: "Mobile accessories",
    postedBy: "Admin",
    contact: "+94767220996",
    contactMail: "raza@gmail.com",
    waLink:
      "https://api.whatsapp.com/send?phone=94767220996&text=Hi, I am looking to buy this Product, ",

    subImg: [
      "https://i1.wp.com/www.laabai.lk/wp-content/uploads/2020/09/Camera-Tripod-3110-3.jpg?resize=555%2C555&ssl=1",
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/09/Camera-Tripod-3110-2.jpg?resize=555%2C555&ssl=1",
      "https://i0.wp.com/www.laabai.lk/wp-content/uploads/2020/09/Camera-Tripod-3110-1.jpg?resize=555%2C555&ssl=1",
    ],
    date: "10-10-2020",
    approvedStatus: "approved",
  },
  {
    id: 2,
    name: "Wireless Koniycoi Magnetic Headphone",
    price: "Rs. 2,700",
    description:
      "Comfort Flexible Neckband Headphone – Around the neck wearing with body-contoured fit, a unique neck-strap and very lightweight(only 1.2oz). The flexible and bendable neck-strap make sure your earbuds are 100% comfortably, especially running, cycling, traveling & staying in the office or at home",
    category: "Mobile accessories",
    postedBy: "Admin",
    contact: "+94767220996",
    contactMail: "raza@gmail.com",
    waLink:
      "https://api.whatsapp.com/send?phone=94767220996&text=Hi, I am looking to buy this Product,",

    subImg: [
      "https://i0.wp.com/www.laabai.lk/wp-content/uploads/2020/09/Wireless-Koniycoi-Magnetic-Headphone-3.jpg?resize=555%2C555&ssl=1",
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/09/Wireless-Koniycoi-Magnetic-Headphone-7.jpg?resize=555%2C555&ssl=1",
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/09/Wireless-Koniycoi-Magnetic-Headphone-5.jpg?resize=555%2C555&ssl=1",
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/09/Wireless-Koniycoi-Magnetic-Headphone-2.jpg?resize=555%2C555&ssl=1",
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/09/Wireless-Koniycoi-Magnetic-Headphone-1.jpg?resize=555%2C555&ssl=1",
    ],
    date: "10-10-2020",
    approvedStatus: "approved",
  },
  {
    id: 3,
    name: "3.5 AUX Lavalier Microphone",
    price: " Rs. 1,300",
    description:
      "100% brand new and high quality Clip Microphone for help you get music or voice. Unidirectional Microphone with anti-noise function. Wind shield improves sound quality by preventing noise. The 3.5mm type jack can be connected to PC / Laptop / wireless microphone transmitter with 3.5mm line-in / mic in. Built-in clip can easily be attached to collars, ties or other clothing-no installation required.",
    category: "Mobile accessories",
    postedBy: "Admin",
    contact: "+94767220996",
    contactMail: "raza@gmail.com",
    waLink:
      "https://api.whatsapp.com/send?phone=94767220996&text=Hi, I am looking to buy this Product,",

    subImg: [
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/09/3.5-AUX-Lavalier-Microphone-5.jpg?resize=555%2C555&ssl=1",
      "https://i0.wp.com/www.laabai.lk/wp-content/uploads/2020/09/3.5-AUX-Lavalier-Microphone-6.jpg?resize=555%2C555&ssl=1",
      "https://i0.wp.com/www.laabai.lk/wp-content/uploads/2020/09/3.5-AUX-Lavalier-Microphone-3.jpg?resize=555%2C555&ssl=1",
      "https://i1.wp.com/www.laabai.lk/wp-content/uploads/2020/09/Camera-Tripod-3110-3.jpg?resize=555%2C555&ssl=1",
    ],
    date: "10-10-2020",
    approvedStatus: "approved",
  },
  {
    id: 4,
    name: "Table Mate Version 2",
    price: "Rs. 1,950",
    description:
      "Few minutes to assemble and disassemble without any tools or fasteners",
    category: "Computer accessories",
    postedBy: "Admin",
    contact: "+94767220996",
    contactMail: "raza@gmail.com",
    waLink:
      "https://api.whatsapp.com/send?phone=94767220996&text=Hi, I am looking to buy this Product,",

    subImg: [
      "https://i1.wp.com/www.laabai.lk/wp-content/uploads/2020/08/Table-Mate-2-3.png?w=800&ssl=1",
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/08/Table-Mate-2-4.jpg?w=800&ssl=1",
    ],
    date: "10-10-2020",
    approvedStatus: "refuse",
  },
  {
    id: 5,
    name: "Laptop Table E-Table With Cooling Fans Stand",
    price: "Rs. 1,999",
    description:
      "Portable & Adjustable Folding Laptop Table E-Table With Cooling Fans Stand – Tablet Accessories Tablet Stands – 1 x Portable Folding Table 1 x USB Cable",
    category: "Computer accessories",
    postedBy: "Admin",
    contact: "+94767220996",
    contactMail: "raza@gmail.com",
    waLink:
      "https://api.whatsapp.com/send?phone=94767220996&text=Hi, I am looking to buy this Product,",

    subImg: [
      "https://i0.wp.com/www.laabai.lk/wp-content/uploads/2020/05/E-Table-01.jpg?resize=555%2C555&ssl=1",
      "https://i0.wp.com/www.laabai.lk/wp-content/uploads/2020/05/E-Table-03.jpg?resize=555%2C555&ssl=1",
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/05/E-Table-08.jpg?resize=555%2C555&ssl=1",
    ],
    date: "10-10-2020",
    approvedStatus: "refuse",
  },
  {
    id: 6,
    name: "Gemei Dryer Model GM 1709",
    price: "Rs. 1,000",
    description:
      "Dry wet hair effectively with the Gemei GM-1709 Hair Dryer that features three heat settings. You can dry different sections of hair as desired with the help of its styling nozzle. This compact dryer can be easily carried in your bag owing to its lightweight design. Featuring a cool-shot function, this portable hair dryer dries your hair without making it frizzy. Foldable Design. 2 Speed Settings & 3 Heat Setting. 220 W Power Consumption. Hanging Loop.",
    category: "Health and Beauty",
    postedBy: "Admin",
    contact: "+94767220996",
    contactMail: "raza@gmail.com",
    waLink:
      "https://api.whatsapp.com/send?phone=94767220996&text=Hi, I am looking to buy this Product,",

    subImg: [
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/05/gemei-1709-01.jpg?resize=555%2C555&ssl=1",
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/05/002-22.jpg?w=500&ssl=1",
      "https://i0.wp.com/www.laabai.lk/wp-content/uploads/2020/05/004-19.jpg?resize=555%2C369&ssl=1",
    ],
    date: "10-10-2020",
    approvedStatus: "refuse",
  },
  {
    id: 7,
    name: "2 in 1 Hair Straightener & Curling Iron GM-2961",
    price: "Rs. 1,750",
    description:
      "The Gemei GM-2961 2 in 1 Hair Straightener & Hair Curling Iron features three specialized plates which enable you to create curly or straight hairstyles.",
    category: "Health and Beauty",
    postedBy: "Admin",
    contact: "+94767220996",
    contactMail: "raza@gmail.com",
    waLink:
      "https://api.whatsapp.com/send?phone=94767220996&text=Hi, I am looking to buy this Product,",

    subImg: [
      "https://i1.wp.com/www.laabai.lk/wp-content/uploads/2019/08/GM-2961-A.jpg?resize=555%2C555&ssl=1",
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2019/08/GM-2961.jpg?w=1080&ssl=1",
    ],
    date: "10-10-2020",
    approvedStatus: "refuse",
  },
  {
    id: 8,
    name: "Ozone National Electric Oven with Rotisserie – 1kg",
    price: "Rs. 7,500",
    description:
      "230/50Hz/2000W/45A Power Indicator Light Bell ring at end of Cooking timer Rotisserie Temperature Control 100°C – 250°C Heat from both Top & Bottom 60minutes Timer with Auto shut-off Heat from Bottom 3 Heat Setting – Heat from Top Color : Orange",
    category: "Kitchen Equipments",
    postedBy: "Admin",
    contact: "+94767220996",
    contactMail: "raza@gmail.com",
    waLink:
      "https://api.whatsapp.com/send?phone=94767220996&text=Hi, I am looking to buy this Product,",

    subImg: [
      "https://i1.wp.com/www.laabai.lk/wp-content/uploads/2020/10/SHX1264-Ozone-National-Electric-Oven-with-Rotisserie-2kg-30L-001-1.jpg?resize=555%2C555&ssl=1",
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/10/Ozone-1kg-Oven-4.jpg?resize=555%2C555&ssl=1",
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/10/Ozone-1kg-Oven-1.jpg?resize=555%2C555&ssl=1",
    ],
    date: "10-10-2020",
    approvedStatus: "refuse",
  },
  {
    id: 9,
    name: "Sunglow 4 Burner Gas Cooker",
    price: "Rs. 15,950",
    description:
      "Sunglow Type: Gas Stove Top Surface Finish : Glass Ignition : Manual Burner Material : Brass Type of Glass: Toughened Glass 7 mm",
    category: "Kitchen Equipments",
    postedBy: "Admin",
    contact: "+94767220996",
    contactMail: "raza@gmail.com",
    waLink:
      "https://api.whatsapp.com/send?phone=94767220996&text=Hi, I am looking to buy this Product,",

    subImg: [
      "https://i0.wp.com/www.laabai.lk/wp-content/uploads/2020/10/SHX-1290-Sunglow-4-Burner-001.jpg?resize=555%2C555&ssl=1",
      "https://i1.wp.com/www.laabai.lk/wp-content/uploads/2020/10/SHX-1290-Sunglow-4-Burner-004.jpg?resize=555%2C555&ssl=1",
      "https://i2.wp.com/www.laabai.lk/wp-content/uploads/2020/10/SHX-1290-Sunglow-4-Burner-003.jpg?resize=555%2C555&ssl=1",
    ],
    date: "10-10-2020",
    approvedStatus: "refuse",
  },
];
